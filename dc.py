...
def tiradado3(website: str, viewsorsubs: int) -> tuple[list[Day], list[int]]:
    subs = []
    dates = []
    string0 = 'TotalSubscribers'
    string1 = 'TotalVideoViews'
    subsdata = ''
    string00 = 'graphdivtotalsubs'
    string11 = 'graphdivtotalviews'
    string = ''
    gj = False
    optionsi = Options()
...
def prolin(cname: str):
    try:
        optionsi = Options()
        optionsi.add_argument("--log-level=3")
        optionsi.add_experimental_option("prefs", {'profile.managed_default_content_settings.images': 2})
        driver = Edge(service=service, options=optionsi)
        driver.get(f'https://google.com/search?q={cname.replace(" ", "+")}+youtube+channel')
...
def propag2(website: str, start=Day(2010, 1, 1), final=Day(2019, 9, 16), suborview=0) -> tuple[list[Day], list[int]]:
    ...
    while ye >= ye1:
        ...
        for i in range(0, len(links)):
            ...
            if greater_or_equal(final + 1, str0) and greater_or_equal(str0, Day(2012, 3, 1)):
                data0.append(tiradado3(links[i].get_attribute('href'), suborview))
                break
        ye -= 1
    driver.quit()
    driver = Edge(service=service, options=optionsi)
...
if __name__ == '__main__':
    tiracodado('https://socialblade.com/youtube/compare/mantovani/tabinho/aldo%20tv')
'''
